...
## Extract Looking time for AOIs from uncomplete dyads (only child or parent were tested)
...
output.file.name <- paste("./data_raw/Output_with_filter/", "Looking_time_to_AOIs_dropouts.txt", sep = "")
...
for (k in 1:nrow(file.names)) {
  ...
  # inner loop: start extracting the AOI looking time data
