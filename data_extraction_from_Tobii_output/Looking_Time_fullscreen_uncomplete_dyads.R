...
## Extract Looking time for Fullscreen from uncomplete dyads (only child or parent were tested)
...
file.names <- read.table("./data_raw/file.names.dropout.txt", sep = "\t", header = T)
...
output.file.name <- paste("./data_raw/Output_with_filter/", "Looking_time_to_Fullscreen_dropouts.txt", sep = "")

# outer loop: prepare extracting the Fullscreen looking time data
for (k in 1:nrow(file.names)) {
  dateiName <- paste("./data_raw/Output_with_filter/", file.names$file[k], ".tsv", sep = "")
  box.name <- file.names$box[k]
  tracking <- read.csv2(dateiName, sep = "\t", header = T, dec = ",", na.strings = c("", " ", "NA"))
  ...
  for (j in 1:nrow(file.orders)) {
    ...
    levels(as.factor(tracking$StudioEventData))
    ...
    for (i in a:b) {
      if (tracking$GazePoint_X[i] > XStart & tracking$GazePoint_X[i] < XEnd & tracking$GazePoint_Y[i] > YStart & tracking$GazePoint_Y[i] < YEnd) {
        Strichliste <- Strichliste + 1
      }
    }

    output$time[min(which(is.na(output$time)))] <- Strichliste
    output$time[min(which(is.na(output$time)))] <- Strichliste
  }
